...
// Sprite drawing ----------------------------------------------------------
// Sprites are simple arrays [size,data]; each entry in data is either a color
// string or 0 for transparency.
export function drawSprite(x,y,def,flip=false){
    const size=def[0];
    const data=def[1];
    for(let i=0;i<data.length;i++){
        ...
        offCtx.fillRect(flip?x+size-px-1:x+px,y+py,1,1);
    }
}
...
export default {
    ...
    randRange,randInt,clamp,lerp,choice,drawSprite,drawText,
    playSFX,shake
};
